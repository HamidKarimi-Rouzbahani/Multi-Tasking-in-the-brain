...
mult_tskrs=[0:0.2:0.8]; % proportion of multi-tasker neurons: neurons which code both features
...
% if label shuffling
% Labels_conds(:,3)=randsample(Labels_conds(:,3),length(Labels_conds(:,3))); % randomize labels

% if controlled similarity across tasks
task_dissim_level=0; % 0 to 8 (Num_conds/2)
...
for shifts=[shiftss]
    ...
    for prp_neurons_mult_tskrs=mult_tskrs
        ...
        for iter=[1:iters]
            ...
            %         Labels_conds(:,3)=Labels_conds(:,2); % variable similarity between tasks
            %         inds_to_toggle=randsample([1:Num_conds],task_dissim_level);
            %         Labels_conds(inds_to_toggle,3)=1-Labels_conds(inds_to_toggle,3);
            %         Data_att_freq=Data_unatt;
            %         Data_att_freq(Neurons_cod_freq,Labels_conds(:,3)==1)=Data_att_freq(Neurons_cod_freq,Labels_conds(:,3)==1)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_cod_freq),sum(Labels_conds(:,3)==1));
            %         Data_att_freq(Neurons_on_freq,:)=Data_att_freq(Neurons_on_freq,:)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_on_freq),size(Labels_conds,1));
            ...
            %         playing with tasks and stimuli
            %         Data_att_freq(Neurons_cod_orient+shifts,Labels_conds(:,3)==1)=Data_att_freq(Neurons_cod_orient+shifts,Labels_conds(:,3)==1)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_cod_orient+shifts),sum(Labels_conds(:,3)==1));
            %         Data_att_freq(Neurons_on_orient+shifts,:)=Data_att_freq(Neurons_on_orient+shifts,:)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_on_orient+shifts),size(Labels_conds,1));
            
            
            %         Data_att_freq=Data_unatt; % same tasks: both orientations
            %         Data_att_freq(Neurons_cod_freq,Labels_conds(:,2)==1)=Data_att_freq(Neurons_cod_freq,Labels_conds(:,2)==1)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_cod_freq),sum(Labels_conds(:,2)==1));
            %         Data_att_freq(Neurons_on_freq,:)=Data_att_freq(Neurons_on_freq,:)+Attentional_bias_freq+Attentiona_vari_freq*randn(length(Neurons_on_freq),size(Labels_conds,1));
            
            %         Data_att_freq=Data_att_orient;
            ...
            %         [rColl(shiftee,1,mt,iter),pColl(shiftee,1,mt,iter)]=corr(reshape(neural_RDM,[sz*sz 1]),reshape(RDM_Conj_Orient_dominant,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rColl(shiftee,2,mt,iter),pColl(shiftee,2,mt,iter)]=corr(reshape(neural_RDM,[sz*sz 1]),reshape(RDM_Conj_Freq_dominant,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rColl(shiftee,3,mt,iter),pColl(shiftee,3,mt,iter)]=corr(reshape(neural_RDM,[sz*sz 1]),reshape(RDM_And,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rColl(shiftee,4,mt,iter),pColl(shiftee,4,mt,iter)]=corr(reshape(neural_RDM,[sz*sz 1]),reshape(RDM_Conj,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rColl(shiftee,5,mt,iter),pColl(shiftee,5,mt,iter)]=corr(reshape(neural_RDM,[sz*sz 1]),reshape(RDM_Or,[sz*sz 1]),'type','pearson','rows','complete');
            %
            %         sz=length(RDM_Freq_Full);
            %         [rAcrossTasks(shiftee,1,mt,iter),pAcrossTasks(shiftee,1,mt,iter)]=corr(reshape(neural_conj,[sz*sz 1]),reshape(RDM_Orient_Full,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rAcrossTasks(shiftee,2,mt,iter),pAcrossTasks(shiftee,2,mt,iter)]=corr(reshape(neural_conj,[sz*sz 1]),reshape(RDM_Freq_Full,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rAcrossTasks(shiftee,3,mt,iter),pAcrossTasks(shiftee,3,mt,iter)]=corr(reshape(neural_conj,[sz*sz 1]),reshape(RDM_mult,[sz*sz 1]),'type','pearson','rows','complete');
            %         [rAcrossTasks(shiftee,4,mt,iter),pAcrossTasks(shiftee,4,mt,iter)]=corr(reshape(neural_conj,[sz*sz 1]),reshape(RDM_Conj(1:sz,sz+1:end),[sz*sz 1]),'type','pearson','rows','complete');
            %         [rAcrossTasks(shiftee,5,mt,iter),pAcrossTasks(shiftee,5,mt,iter)]=corr(reshape(neural_conj,[sz*sz 1]),reshape(RDM_sum,[sz*sz 1]),'type','pearson','rows','complete');
